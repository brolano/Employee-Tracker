...
async function addRole() {
    ...
    const departmentChoices = department.rows.map((dept: any) => ({
        name: dept.name,
        value: dept.id,
    }));
...
async function addEmployee() {
    ...
    const roleChoices = role.rows.map((role: any) => ({
        name: role.title,
        value: role.id,
    }));

    const employee = await pool.query('SELECT * FROM employee');
    const managerChoices = employee.rows.map((emp: any) => ({
...
async function updateEmployeeRole() {
    const employee = await pool.query('SELECT * FROM employee');
    const employeeChoices = employee.rows.map((emp: any) => ({
        name: `${emp.first_name} ${emp.last_name}`,
        value: emp.id,
    }));
